...
int main(int argc, char* argv[])
{
...

	{
		...
		if ( 0 == UCI_OPEN(uci_req) )
		{
			...
			{
				SAFE_SPRINTF(u_type, "type_%02d", s_num);
				SAFE_SPRINTF(u_name, "name_%02d", s_num);
...

				{
					...
					{
						SAFE_SPRINTF(u_key, "key_%02d", idx);
						SAFE_SPRINTF(u_val, "%02d", idx);
...
#if (1)
			{
				char u_name[LEN_OF_BUF128] = "";
				char u_type[LEN_OF_BUF128] = "";
				SAFE_SPRINTF(u_type, "type_%02d", 4);
				SAFE_SPRINTF(u_name, "name_%02d", 4);
				//struct uci_section *u_section = UCI_SESSION_SET(uci_req, u_type, u_name);

				int idx = 0;
				char u_key[LEN_OF_BUF128] = "";
				char u_val[LEN_OF_BUF128] = "";
				SAFE_SPRINTF(u_key, "key_%02d", 1);
				for ( idx = 0; idx < 5; idx++ )
				{
					SAFE_SPRINTF(u_val, "%02d", idx);
