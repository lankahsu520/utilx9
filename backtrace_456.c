...
void crash_it(char *const payload, int len)
{
	int i;
	for (i = 0; i < len+1; ++i)
	{
...
void function_deep_2(void)
{
	char payload[] = { 0x00, 0x01, 0x02, 0x03};
	int len = sizeof(payload);
	
	crash_it(payload, len);
	crash_it(payload, len+1);
	crash_it(payload, len);
}

void function_deep_1(void)
{
	function_deep_2();
}
