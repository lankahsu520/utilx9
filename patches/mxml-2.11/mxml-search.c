...
mxml_node_t *				/* O - Element node or @code NULL@ */
mxmlFindElement_ex(mxml_node_t *node,	/* I - Current node */
    mxml_node_t *top,	/* I - Top node */
    const char  *ns,	/* I - namespace or @code NULL@ for any */
    const char  *element,	/* I - Element name or @code NULL@ for any */
		const char  *attr,	/* I - Attribute name, or @code NULL@ for none */
		const char  *value,	/* I - Attribute value, or @code NULL@ for any */
		int         descend)	/* I - Descend into tree - @code MXML_DESCEND@, @code MXML_NO_DESCEND@, or @code MXML_DESCEND_FIRST@ */
{
  ...
  */
  ...
  while (node != NULL)
  {
    ...
    */
		char *node_ns = NULL;
		char *node_name = node->value.element.name;
		if ((node->type == MXML_ELEMENT) && (node_name))
		{
			if ( (node_ns = strchr(node->value.element.name, ':' ) ) &&
				(strlen(node_ns)>=2) )
			{
				node_ns = node_ns+1;
			}
    ...
    if ((node->type == MXML_ELEMENT) &&
        (node_name) &&
				(!ns || !strncmp(node_name, ns, strlen(ns))) &&
				(!element || !strcmp(node_ns, element))
				)
    {
     /*
      * See if we need to check for an attribute...
